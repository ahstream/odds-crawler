...
export async function processAsyncFeeds(event) {
  ...
  const feedsToFetch = [];
  Object.keys(event.bettingTypes).forEach((bettingTypeKey, _bettingTypeIndex) => {
    Object.keys(event.bettingTypes[bettingTypeKey]).forEach((scopeKey, _scopeIndex) => {
...
function getFeedByHtml(event, htmltext) {
  ...
  if (!scrapedFeed || scrapedFeed.length != 2) {
    ...
    log.verbose('getFeedByHtml error, htmltext:', htmltext);
    return null;
  }
...
function processFeed(event, feed) {
  ...
  Object.keys(feed.oddsdata).forEach((backOrLayKey, _index) => {
    betlib.processBets(event, feed, feed.oddsdata[backOrLayKey], backOrLayKey, feed.bt, feed.sc);
  });
