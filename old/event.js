...
export async function getEventFromWebPage(url, season) {
  try {
    ...
    event.bettingTypes = await betlib.getBetTypes(event);
    ...
    const feedsResult = await feedlib.processMatchFeeds(event);
...
export async function crawlEvent(
  ...
  try {
    ...
    if (!event.ok && options.addToQueue) {
      ...
      crawlerQueue.addEventToQueue(url, divisionCode, season, {
        comment: event.error
      });
    }
...
async function processScore(event) {
  ...
  event.startTimeMs = score.startTimeMs;
...
function processBaseEvent(event) {
  ...
  ptr.startTimeMs = event.startTimeMs;
