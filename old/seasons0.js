...
const httplib = require('../src/lib/httplib');
...
export async function getSeason(
  ...
  year,
  url = '',
  { flCreateSeasonPages = true }
) {
  divisionCode = utilslib.trimCharsRight(divisionCode, '/');
...
export async function getSeasons(divisionCode, flCompleteSeasons = false) {
  ...
  const response = await httplib.getResponse(
    url,
    httpRequestConfig,
    config.delayBetweenHttpRequests
  );
  ...
  if (!scrapedDivisionName || scrapedDivisionName.length != 2) {
    log.error(
      `Failed to scrape division name for division ${divisionCode}, scrapedDivisionName: ${scrapedDivisionName}`
    );
    return seasons;
  }
  ...
  if (divisionName.length <= 0) {
    log.error(
      `Scraped division name is empty for division: ${divisionCode}, divisionName: ${divisionName}`
    );
    return seasons;
  }

  const reSeasons =
    /<li><span class="(active|inactive)"><strong><a href="([^"]*)">([0-9]*)\/?([0-9]*)/gim;
  ...
  if (seasons.length <= 0) {
    log.error(
      `No valid seasons for division: ${divisionCode}, scrapedSeasons: ${scrapedSeasons}`
    );
    return seasons;
  }
  ...
  for (let i = 0; i < seasons.length; i++) {
    ...
    if (seasonPages.length == 0) {
      log.error(
        `Failed to create season pages for season year: ${season.seasonYear} for division: ${divisionCode}`
      );
      season.ok = false;
...
async function getSeasonId(url) {
  ...
  if (!scrapedSeason || scrapedSeason.length != 2) {
    log.error(
      `Failed to get season id params for season: ${url}, scrapedSeason: ${scrapedSeason}`
    );
    return '';
  }
  ...
  try {
    ...
    log.error(
      `Failed to json parse season id params for season: ${url}, scrapedSeason: ${scrapedSeason}`
    );
    return '';
  }
  if (!seasonParams || !seasonParams.id || seasonParams.id.length <= 0) {
    log.error(
      `Failed to get seasonParams.id params for season: ${url}, scrapedSeason: ${scrapedSeason}`
    );
    return '';
  }
...
async function getSeasonPage(season, pageNum, flAddEventUrls) {
  log.debug(
    `Get season page: ${pageNum}, season: ${season.seasonName}, division: ${season.divisionCode}`
  );
  ...
  if (season.seasonId === null) {
    ...
    if (seasonId.length <= 0) {
      log.error(
        `Failed to get valid seasonId: ${seasonId}, season: ${season.seasonName}`
      );
      return seasonPage;
    }
  ...
  if (!httplib.isSuccess(response, dataUrl)) {
    log.error(
      `Failed to get URL season data: ${season.seasonName}, dataUrl: ${dataUrl}`
    );
  ...
  const reLastPage =
    /x-page=\\"([0-9]*)\\"><span class=\\"arrow\\">&raquo;\|<\\\/span><\\\/a><\\\/div>"[^}]*},[^"]*"refresh":/im;
   ...
   */
  const reHistoricDivision =
    /<span class=\\"bflp\\">\\u00bb<\\\/span><a href=\\"[^"]*\">([^<]*)/im;
  ...
  if (!scrapedHistoricDivision || scrapedHistoricDivision.length != 2) {
    ...
    log.error(
      `Failed to get historicDivisionName for season: ${season.seasonName}, scrapedHistoricDivision: ${scrapedHistoricDivision}`
    );
  ...
  const historicDivisionName = scrapedHistoricDivisionHtml
    .trim()
    .replace('\\/', '')
    .replace(/\d+$/, '')
    .trim();
  if (historicDivisionName.length <= 0) {
    log.error(
      `Historic division name is empty for season: ${season.seasonName}, scrapedHistoricDivisionHtml: ${scrapedHistoricDivisionHtml}`
    );
    log.verbose(`htmlResult: ${htmlResult}`);
    return seasonPage;
  }
  ...
  if (scrapedEventDates && numEventDates > 0) {
    seasonPage.lastEventDate = tools.convertTimestampToDate(
      parseInt(scrapedEventDates[0][1]),
      10
    );
    seasonPage.firstEventDate = tools.convertTimestampToDate(
      parseInt(scrapedEventDates[numEventDates - 1][1], 10)
    );
  } else {
    log.error(
      `Season page does not contain any events for season: ${season.seasonName}, scrapedEventDates: ${scrapedEventDates}`
    );
    log.verbose(`htmlResult: ${htmlResult}`);
    return seasonPage;
  }
  ...
  if (flAddEventUrls) {
    const reEventUrls =
      /<td class=\\"name table-participant\\"><a href=\\"([^"]*)\\">/gim;
    const scrapedEventUrls = [...htmlResult.matchAll(reEventUrls)];
    const eventUrls = [];
    for (let i = 0; i < scrapedEventUrls.length; i++) {
      eventUrls.push(
        `https://www.oddsportal.com${scrapedEventUrls[i][1].replaceAll(
          '\\',
          ''
        )}`
      );
...
async function getSeasonPages(season, flAddEventUrls) {
  log.debug(
    `Get all season pages for season: ${season.seasonName}, division: ${season.divisionCode}`
  );
...
async function createSeasonPages(season) {
  ...
  if (
    ...
    (seasonPages.length == 1 && seasonPages[0].noDataAvailable)
  ) {
    return seasonPages;
  }
...
function createSeason(divisionCode, divisionName, scrapedSeason) {
  ...
  newSeason.seasonYear = Number.isInteger(year2)
    ? year2
