...
function getDivisionData(event) {
  ...
  const scrapedParams = scrapeUtils.matchText(event.html, /<h2 class=\"out\">You are here<\/h2>[^<]*<[^>]*>Home<\/a>[^<]*<a href=\"\/[^\/]*\/\">([^<]*)<\/a>[^<]*<a href=\"[^\"]*\">([^<]*)<\/a>[^<]*<a href=\"[^\"]*\">([^<]*)<\/a>/im);
...
async function getAllFeeds(event) {
  const feedsToFetch = [];
  Object.keys(config.bettingTypes).forEach((bettingTypeKey, _bettingTypeIndex) => {
    Object.keys(config.scopes).forEach((scopeKey, _scopeIndex) => {
      feedsToFetch.push({
        bettingTypeId: config.bettingTypes[bettingTypeKey].id,
        scopeId: config.scopes[scopeKey].id
      });
    });
  });
...
async function createEventScore(event) {
  ...
  const scrapedResult = scrapeUtils.matchText(jsonScore.result, /\<strong\>([0-9]+)\:([0-9]+)\<\/strong\> \(([0-9]+)\:([0-9]+)\, ([0-9]+)\:([0-9]+)\)/im);
...
async function createEventBettingTypes(event) {
  ...
  const bettingTypes = {};
  Object.keys(feedData.nav).forEach((bettingTypeKey, _index) => {
    utils.ensureProperty(bettingTypes, bettingTypeKey);
    Object.keys(feedData.nav[bettingTypeKey]).forEach((scopeKey, _scopeIndex) => {
...
function createEventOdds(event, feed) {
  ...
  Object.keys(feed.oddsdata).forEach((backOrLayKey, _index) => {
...
function processBackOrLay(event, eventData, feedData, bt, sc) {
  const bets = [];
  Object.keys(feedData).forEach((betKey, _index) => {
  ...
  bets.forEach(({ betKey, _betAttribute, _isMixedParameter }) => {
...
function processBet(event, eventData, feedData, bt, sc, isBack, betAttribute) {
  ...
  Object.keys(feedData.OutcomeID).forEach((outcomeKey, _outcomeIndex) => {
...
function processOpeningOrClosingOdds(isOpening, event, eventData, feedData, outcomeKey, bt, sc, isBack, betAttribute) {
  ...
  Object.keys(oddsPointer).forEach((bookieKey, _index) => {
    const realOutcome = parseInt(outcomeKey, 10) + 1;
    const realOutcomeKey = realOutcome.toString();
    const betKey = `${bt.toString()}_${sc.toString()}_${isBack ? 'back' : 'lay'}_${betAttribute}_${realOutcomeKey}_${bookieKey}`;
    ...
    if (isOpening) {
      ...
      event.oddsData[betKey].openingVolume = volumePointer[bookieKey] && volumePointer[bookieKey][outcomeKey] ? volumePointer[bookieKey][outcomeKey] : 0;
    } else {
      ...
      event.oddsData[betKey].closingVolume = volumePointer[bookieKey] && volumePointer[bookieKey][outcomeKey] ? volumePointer[bookieKey][outcomeKey] : 0;
    }
    ...
    if (isOpening) {
      ...
      eventData[outcomeKey][bookieKey].openingVolume = volumePointer[bookieKey] && volumePointer[bookieKey][outcomeKey] ? volumePointer[bookieKey][outcomeKey] : 0;
    } else {
      ...
      eventData[outcomeKey][bookieKey].closingVolume = volumePointer[bookieKey] && volumePointer[bookieKey][outcomeKey] ? volumePointer[bookieKey][outcomeKey] : 0;
    }
  });
}
