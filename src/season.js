...
export async function getSeasonFromWebPage(url, { flGetAllSeasonInfo = true }) {
  try {
    ...
    const seasonLinks = parser.parseSeasonLinksFromSeasonPage(htmltext, divisionName);
    ...
    if (flGetAllSeasonInfo) {
      ...
      numEventsTotal = seasonsInfo.map((item) => item.numEvents).reduce((sum, val) => sum + val, 0);
      numEventsPerSeason = _.round(numEventsTotal / numSeasons, 0);
      numBookiesPerSeason = _.round(seasonsInfo.map((item) => item.numBookies).reduce((sum, val) => sum + val, 0) / numSeasons, 0);
    }

    const seasonId = parser.parseSeasonIdFromSeasonPage(htmltext);
    assert(seasonId, 'seasonId falsey');

    const seasonArchivePageHtml = await getOneSeasonOneArchivePageHtml(seasonId, 1);
    assert(seasonArchivePageHtml, 'seasonArchivePageHtml falsey');

    const historicDivisionName = parser.parseSeasonHistoricDivisionName(seasonArchivePageHtml);

    const bookies = parser.parseNumberOfBookies(seasonArchivePageHtml, { minmax: true });
    assert(bookies.length === 3, 'parseNumberOfBookies !== 3');

    const pagination = parser.parsePaginationFromSeasonArchive(seasonArchivePageHtml);

    const pagenums = Array.from({ length: pagination.lastPage }, (_, i) => i + 1);
    const seasonArchivePagesHtml = await getOneSeasonManyArchivePagesHtml(seasonId, pagenums);
    ...
    seasonArchivePagesHtml.forEach((pagehtml) => {
      ...
      numEvents = numEvents + pageEventLinks.length;
    });
...
async function getOneSeasonOneArchivePageHtml(seasonId, pagenum = 1) {
  try {
    ...
    log.error(`getSeasonArchiveHtml exception: ${error}, seasonId: ${seasonId}, pagenum: ${pagenum}, stack: ${error.stack}`);
    return null;
  }
}
...
async function getOneSeasonManyArchivePagesHtml(seasonId, pagenums) {
  try {
    ...
    log.error(`getOneSeasonManyArchivePagesHtml exception: ${error}, seasonId: ${seasonId}, pagenum: ${pagenum}, stack: ${error.stack}`);
    return null;
  }
}
...
async function getManySeasonsOneArchivePageHtml(seasonIds) {
  try {
    ...
    log.error(`getManySeasonsOneArchivePageHtml exception: ${error}, seasonIds: ${seasonIds}, stack: ${error.stack}`);
    return null;
  }
}
...
async function getSeasonInfoFromSeasonPages(seasonPageLinks) {
  ...
  const manySeasonsArchivePageHtml = await getManySeasonsOneArchivePageHtml(seasonIds);
