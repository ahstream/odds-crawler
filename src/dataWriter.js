  ...
  switch (key.toLowerCase()) {
    case 'event':
      if (divisionCode === null) {
        ...
        filelib.emptyFolder(`${config.pathToDataFolder}/result/event/${divisionCodeToFileSafeName(divisionCode)}`, false);
      }
      break;
    case 'bookie':
...
function ensureDivisionFolder(divisionCode) {
  const path = `${config.pathToDataFolder}/result/event/${divisionCodeToFileSafeName(divisionCode)}`;
...
function convertToSqlSafeCsvData(data) {
  Object.keys(data).forEach(function (key, _index) {
    if (typeof data[key] !== 'object' || data[key] === null) {
      ...
      Object.keys(data[key]).forEach(function (key2, _index2) {
