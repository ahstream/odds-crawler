...
const seasonlib = require('./season.js');
...
async function processEventItem(item) {
  ...
  if (rand > 0.9) {
    event.error = 'Random error: ' + rand;
  }
  */
  if (event.error) {
    addEvent(item.url, item.divisionCode, item.year, { ...item, ...{ error: event.error } });
  }
}
...
async function processSeasonItem(item) {
  ...
  if (!season.error) {
    ...
    removeItemFromQueue(item);
    await processQueueByOptions({ type: 'event', divisionCode: season.divisionCode, year: season.year });
  } else {
    removeItemFromQueue(item);
    addSeason(item.url, item.divisionCode, { ...item, ...{ error: season.error } });
  }
}
...
async function processDivisionItem(item) {
  ...
  if (!firstSeason.error) {
    ...
    removeItemFromQueue(item);
    await processQueueByOptions({ type: 'season', divisionCode: firstSeason.divisionCode });
  } else {
    removeItemFromQueue(item);
    addDivision(item.url, item.divisionCode, { ...item, ...{ error: firstSeason.error } });
  }
}
...
async function processCountryItem(item) {
  ...
  if (!divisions.error) {
    ...
    removeItemFromQueue(item);
    addCountry(item.url, item.sport, item.country, { ...item, ...{ error: divisions.error } });
  }
}
...
export function addEvent(url, divisionCode, year, options = {}) {
  const baseOptions = { type: 'event', url: ensureUrlEndingSlash(url), divisionCode, year };
  return addItemToQueue({ ...baseOptions, ...options });
}

export function addSeason(url, divisionCode, options = {}) {
  const baseOptions = { type: 'season', url: ensureUrlEndingSlash(url), divisionCode };
  return addItemToQueue({ ...baseOptions, ...options });
}
...
export function addCountry(url, sport, country, options = {}) {
  const baseOptions = { type: 'country', sport, country, url: ensureUrlEndingSlash(url) };
  return addItemToQueue({ ...baseOptions, ...options });
}
...
function ensureUrlEndingSlash(url) {
  return utilslib.trimRightChars(url, '/') + '/';
}
