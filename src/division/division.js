...
export async function getDivisionsFromWebPage(url) {
  try {
    ...
    log.error(
      `getDivisionsFromWebPage exception: ${error}, url: ${url}, stack: ${error.stack}`
    );
...
export function parseDivisionData(url, htmltext, fallbackSeason = null) {
  ...
  if (!scrapedDivision || scrapedDivision.length != 4) {
    log.error(
      `Failed to scrape division data for event: ${url}, scrapedDivision: ${scrapedDivision}`
    );
    return null;
  }
  ...
  if (divisionData.season === null) {
    ...
    log.debug(
      `Failed to scrape division season year for event: ${url}, set season year from fallbackSeason: ${fallbackSeason}`
    );
  }
...
export function validateDivisionData(divisionData, url) {
  ...
  if (
    divisionData.sport.replaceAll(' ', '').toLowerCase() !==
    parsedUrl.sport.replaceAll('-', '').toLowerCase()
  ) {
    log.error(parsedUrl);
    return false;
  }
  if (
    divisionData.country.replaceAll(' ', '').toLowerCase() !==
    parsedUrl.country.replaceAll('-', '').toLowerCase()
  ) {
    log.error(parsedUrl);
    return false;
  }
  if (
    divisionData.division.replaceAll(' ', '').toLowerCase() !==
    parsedUrl.divisionCodeName.replaceAll('-', '').toLowerCase()
  ) {
    log.error(parsedUrl);
    return false;
  }
