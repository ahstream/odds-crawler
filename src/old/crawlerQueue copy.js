...
export async function crawlQueue(queueOptions = {}, { flReCrawl = true, flPreCrawl = true, skipDups = true }) {
  ...
  log.info(`Crawl queue with queueOptions: ${JSON.stringify({ queueOptions })}, options: ${JSON.stringify({ options })}`);
    ...
    switch (queueItem.type) {
      case 'event':
        ...
        if (event.duplicate) {
          ...
          removeFromQueue(queueItem);
          addEvent(null, null, null, { ...queueItem, ...{ error: event.error } });
        }
        break;
      case 'season':
        ...
        if (season.ok) {
          ...
          removeFromQueue(queueItem);
          await crawlQueue({ type: 'event', divisionCode: season.divisionCode, year: queueItem.year }, nextOptions);
